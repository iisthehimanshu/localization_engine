...
class DirectionalLocalisation {
  ...
  Future<Pt?> estimateIntersectionCenter(
    ...
    beaconData.removeWhere((key, value) => apibeaconmap[key] == null);
...
// 3️⃣ Select top 3 strongest beacons
    final sortedBeacons = avgRssi.entries.toList()
      ..sort((a, b) => b.value.compareTo(a.value));

    if (sortedBeacons.isEmpty) {
      return null;
    } else if (sortedBeacons.length == 1) {
      Beacon beacon = apibeaconmap[sortedBeacons.first.key]!;
      return Pt(x: beacon.coordinateX!.toDouble(), y: beacon.coordinateY!.toDouble(), beacon: beacon, top3beacons: [beacon]);
    } else if (sortedBeacons.length == 2) {
      final beaconKeys = sortedBeacons.map((e) => e.key).take(2).toList();
      beaconKeys.add(beaconKeys.last); // Duplicate for compatibility
    }

    final topBeaconKey = sortedBeacons.first.key;
    final topBeaconPos = Point(
        apibeaconmap[topBeaconKey]!.coordinateX!.toDouble(),
        apibeaconmap[topBeaconKey]!.coordinateY!.toDouble()
    );
    final topBeaconBuildingID = apibeaconmap[topBeaconKey]!.buildingID;

    // Find all possible collinear sets that include the top beacon
    List<List<String>> collinearSets = [];

// Check all pairs of other beacons with the top beacon
    for (int i = 1; i < sortedBeacons.length; i++) {
      final beacon1Key = sortedBeacons[i].key;
      if (apibeaconmap[beacon1Key]!.buildingID != topBeaconBuildingID) continue;
      for (int j = i + 1; j < sortedBeacons.length; j++) {
        final beacon2Key = sortedBeacons[j].key;
        if (apibeaconmap[beacon2Key]!.buildingID != topBeaconBuildingID) continue;


        final beacon1Pos = Point(
            apibeaconmap[beacon1Key]!.coordinateX!.toDouble(),
            apibeaconmap[beacon1Key]!.coordinateY!.toDouble()
        );
        final beacon2Pos = Point(
            apibeaconmap[beacon2Key]!.coordinateX!.toDouble(),
            apibeaconmap[beacon2Key]!.coordinateY!.toDouble()
        );

        // Check if these three beacons are collinear
        if (checkCollinearity(a: topBeaconPos, b: beacon1Pos, c: beacon2Pos)) {
          // Found a collinear set, now find all other beacons collinear with this line
          List<String> currentSet = [topBeaconKey, beacon1Key, beacon2Key];

          // Check remaining beacons
          for (int k = 1; k < sortedBeacons.length; k++) {
            if (k == i || k == j) continue;

            final beaconKKey = sortedBeacons[k].key;
            if (apibeaconmap[beaconKKey]!.buildingID != topBeaconBuildingID) continue;
            final beaconKPos = Point(
                apibeaconmap[beaconKKey]!.coordinateX!.toDouble(),
                apibeaconmap[beaconKKey]!.coordinateY!.toDouble()
            );

            // Check if this beacon is collinear with the line
            if (checkCollinearity(a: topBeaconPos, b: beacon1Pos, c: beaconKPos)) {
              currentSet.add(beaconKKey);
            }
          }

          collinearSets.add(currentSet);
        }
      }
    }

// Select the largest collinear set
    List<String> bestCollinearSet = [topBeaconKey];
    if (collinearSets.isNotEmpty) {
      collinearSets.sort((a, b) => b.length.compareTo(a.length));
      bestCollinearSet = collinearSets.first;
      print("Found ${collinearSets.length} collinear sets, largest has ${bestCollinearSet.length} beacons");
    }

// If we couldn't find any collinear set with 3+ beacons, fall back to top beacon
    if (bestCollinearSet.length < 3) {
      Beacon beacon = apibeaconmap[topBeaconKey]!;
      return Pt(x: beacon.coordinateX!.toDouble(), y: beacon.coordinateY!.toDouble(), beacon: beacon,
          top3beacons: [beacon]);
    }

// Sort the best collinear set by signal strength and take top 3
    final bestSetWithRssi = bestCollinearSet.map((key) => MapEntry(key, avgRssi[key]!)).toList()
      ..sort((a, b) => b.value.compareTo(a.value));

    final beaconKeys = bestSetWithRssi.take(3).map((e) => e.key).toList();

// Ensure we have exactly 3
    if (beaconKeys.length == 2) {
      beaconKeys.add(beaconKeys.last);
    }
...
// 5️⃣ Extract positions of top 2 beacons
    ...
    if (angleBetweenTopBeaconAndSecondBeacon - 90 < userDirection && angleBetweenTopBeaconAndSecondBeacon + 90 > userDirection) {
...
// 9️⃣ Return candidate with smaller error
    ...
    if (probPtBetween > probPtAway) {
      return Pt(x: ptBetween.x.toDouble(), y: ptBetween.y.toDouble(), beacon: apibeaconmap[beaconKeys.first]!, top3beacons: top3Beacons);
    } else {
      return Pt(x: ptAway.x.toDouble(), y: ptAway.y.toDouble(), beacon: apibeaconmap[beaconKeys.first]!, top3beacons: top3Beacons);
    }
  }
