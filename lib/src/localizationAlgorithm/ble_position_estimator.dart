...
class _Agg {
  ...
  _Agg({
    ...
    required this.peak, required this.sumRssi, required this.n,
  });
  double get meanRssi => sumRssi / n;
}
...
class BLEPositionEstimator {
  ...
  PositionResult? update(List<BleReading> readings, {bool? walking}) {
    ...
    for (final e in _buf) {
      ...
      if (agg.containsKey(e.name)) {
        ...
        agg[e.name] = _Agg(
          ...
          peak: e.rssi, sumRssi: e.rssi, n: 1,
        );
      }
    }
    ...
    // meanRssi instead of peak: positioning weights should reflect how close
    // a beacon *typically* read this window, not its single loudest ping.
  ...
  static String _floorConf(double m) =>
      m >= 12.0 ? 'high' : m >= _floorSwitchMargin ? 'medium' : 'low';
